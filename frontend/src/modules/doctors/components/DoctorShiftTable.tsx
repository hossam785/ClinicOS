...
export interface DoctorShiftTableProps {
  shifts: DoctorShift[]
  editable?: boolean
  onShiftsChange?: (updatedShifts: DoctorShift[]) => void
}

export default function DoctorShiftTable({
  shifts,
  editable = false,
  onShiftsChange,
}: DoctorShiftTableProps) {
  const handleToggleOpen = (index: number) => {
    if (!editable || !onShiftsChange) return
      ...
      >
        <thead>
          ...
          >
            ...
            <th style={{ padding: '0.75rem 1rem' }}>Break Interval</th>
          </tr>
        </thead>
        <tbody>
          {shifts.map((day, idx) => (
                ...
                {editable ? (
                  ...
                  >
                    <Badge variant={day.isOpen ? 'success' : 'neutral'}>
                      {day.isOpen ? 'Active Shift' : 'Off / No Shift'}
                    </Badge>
                  </button>
                ) : (
                  <Badge variant={day.isOpen ? 'success' : 'neutral'}>
